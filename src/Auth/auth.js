    ...
    await fetch(`${backendUrl}/signIn`, requestOptions)
      ...
      .then(async (data) => {
        for (let i = 0; i < data.data.length; i++) {
          console.log("data.data[0].email", data.data[i].userType);
        }
        if (data.status == true) {
          try {
          ...
          for (let i = 0; i < data.data.length; i++) {
            if (data.data[i].userType == 'Mentees') {
              props.navigation.navigate("mentor", { profile: data });
            }
            else { props.navigation.navigate("mentees", { profile: data }); }
          }
